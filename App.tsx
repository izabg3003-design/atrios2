    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden relative">
      ...
      {view === 'landing' ? (
        ...
        <div className="flex h-screen bg-slate-50 overflow-hidden relative w-full">
          ...
          <aside className={`fixed inset-y-0 left-0 w-80 bg-white border-r border-slate-100 flex flex-col shrink-0 shadow-2xl lg:shadow-sm z-[70] transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="p-8 lg:p-10">
              <div className="flex items-center justify-between mb-12 lg:mb-16">
                <div className="flex items-center gap-3">
                  <div className="bg-amber-500 p-2 rounded-xl"><Construction className="text-white" size={24} /></div>
                  <span className="text-2xl lg:text-3xl font-black tracking-tighter">{t.appName}</span>
                </div>
                ...
                {[
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full flex items-center gap-4 lg:gap-5 px-5 lg:px-6 py-3.5 lg:py-4 rounded-[1.25rem] lg:rounded-[1.5rem] font-black transition-all ${activeTab === item.id ? 'bg-slate-900 text-white shadow-2xl' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
            ...
            <div className="mt-auto p-6 border-t border-slate-50 space-y-2">
              <div className="bg-slate-50 p-4 rounded-2xl lg:rounded-3xl flex items-center gap-3 relative">
                <div className="w-10 h-10 lg:w-12 lg:h-12 bg-slate-900 rounded-xl lg:rounded-2xl flex items-center justify-center font-black text-white uppercase overflow-hidden shrink-0">
                  {currentUser?.logo ? <img src={currentUser.logo} className="w-full h-full object-cover" alt="Logo" /> : currentUser?.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-slate-900 truncate text-xs lg:text-sm">{currentUser?.name}</p>
                  <p className="text-[10px] lg:text-xs font-bold text-slate-400 uppercase tracking-tighter">{getTranslatedPlan(currentUser?.plan || PlanType.FREE)}</p>
                </div>
                {unreadCount > 0 && (
              ...
              <button 
                ...
                className="w-full flex items-center gap-4 px-6 py-3 text-slate-400 hover:text-red-500 transition-colors font-black uppercase tracking-widest text-[9px] lg:text-[10px]"
              >
          ...
          <main className="flex-1 flex flex-col overflow-hidden w-full">
            <header className="h-20 lg:h-24 bg-white border-b border-slate-100 flex items-center justify-between px-4 lg:px-12 shrink-0 gap-4">
              <div className="flex items-center gap-3 lg:hidden">
                ...
                <span className="text-xl font-black tracking-tighter">{t.appName}</span>
              </div>

              <div className="hidden sm:flex items-center bg-slate-50 rounded-xl lg:rounded-2xl px-4 lg:px-6 py-2.5 lg:py-3 w-full max-w-[16rem] lg:max-w-[28rem] border border-slate-100 focus-within:border-slate-300 transition-all">
              ...
              <div className="flex items-center gap-2 lg:gap-8">
                <div className="hidden md:block bg-slate-900 rounded-xl p-0.5"><Selectors /></div>
                <button 
                  onClick={() => { setSelectedBudget(undefined); setIsEditingBudget(true); }} 
                  className="px-4 lg:px-8 py-2.5 lg:py-4 bg-slate-900 text-white rounded-xl lg:rounded-[1.5rem] font-black flex items-center gap-2 lg:gap-3 hover:bg-slate-800 transition-all shadow-2xl text-xs lg:text-base"
                >
                              ...
                              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-10">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex flex-wrap items-center gap-2 lg:gap-3 mb-1 lg:mb-2">
                                    <h4 className="font-black text-slate-900 text-lg lg:text-xl truncate">{budget.clientName}</h4>
                                    <span className="text-[8px] lg:text-[10px] font-black bg-slate-100 text-slate-500 px-2 lg:px-3 py-0.5 lg:py-1 rounded-full border border-slate-200 tracking-tighter uppercase">{t.budgetRef}: {budget.id}</span>
                                    <span className={`text-[8px] lg:text-[10px] font-black px-2 lg:px-3 py-0.5 lg:py-1 rounded-full border tracking-tighter uppercase ${budget.status === BudgetStatus.APPROVED ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : budget.status === BudgetStatus.REJECTED ? 'bg-red-100 text-red-700 border-red-200' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>{getTranslatedStatus(budget.status)}</span>
                                  </div>
                                  <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-[8px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest lg:tracking-[0.2em]">
                                    <span className="flex items-center gap-1.5 lg:gap-2"><User size={12} className="lg:w-3.5 lg:h-3.5" /> {budget.contactName}</span>
                                    <span className="flex items-center gap-1.5 lg:gap-2"><Clock size={12} className="lg:w-3.5 lg:h-3.5" /> {new Date(budget.createdAt).toLocaleDateString(locale)}</span>
                                  </div>
                                </div>
                                <div className="sm:text-right w-full sm:w-auto border-t sm:border-t-0 border-slate-50 pt-3 sm:pt-0">
                                  <p className="text-[8px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 lg:mb-1">{t.total}</p>
                                  <p className="text-xl lg:text-3xl font-black text-slate-900">{(budget.totalAmount * CURRENCIES[currencyCode].rate).toLocaleString(locale, { style: 'currency', currency: currencyCode })}</p>
                                </div>
                                <div className="flex gap-2 lg:gap-3 sm:opacity-0 sm:group-hover:opacity-100 transition-all transform sm:translate-x-2 sm:group-hover:translate-x-0 w-full sm:w-auto justify-end">
                                  <button onClick={() => { setSelectedBudget(budget); setShowPaymentManager(true); }} className="p-3 lg:p-4 bg-emerald-50 text-emerald-600 rounded-xl lg:rounded-2xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm"><PaymentIcon size={18} className="lg:w-[22px] lg:h-[22px]" /></button>
                                  <button onClick={() => { setSelectedBudget(budget); setShowExpenseManager(true); }} className="p-3 lg:p-4 bg-red-50 text-red-600 rounded-xl lg:rounded-2xl hover:bg-red-600 hover:text-white transition-all shadow-sm"><Wallet size={18} className="lg:w-[22px] lg:h-[22px]" /></button>
                                  <button onClick={() => exportToPDF(budget)} className="p-3 lg:p-4 bg-blue-50 text-blue-600 rounded-xl lg:rounded-2xl hover:bg-blue-600 hover:text-white transition-all shadow-sm"><Download size={18} className="lg:w-[22px] lg:h-[22px]" /></button>
                                  <button onClick={() => { setSelectedBudget(budget); setIsEditingBudget(true); }} className="p-3 lg:p-4 bg-slate-50 text-slate-900 rounded-xl lg:rounded-2xl hover:bg-slate-900 hover:text-white transition-all shadow-sm"><ChevronRight size={18} className="lg:w-[22px] lg:h-[22px]" /></button>
                                </div>
                              </div>
                            </div>
                          ))
