...
const App: React.FC = () => {
  ...
  const [expiryAlertDismissed, setExpiryAlertDismissed] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('atrios_expiry_banner_dismissed') === 'true';
    }
    return false;
  });
  ...
  const [pushSoundEnabled, setPushSoundEnabledState] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
  ...
  const handleTogglePushSound = () => {
    const nextVal = !pushSoundEnabled;
    setPushSoundEnabledState(nextVal);
    if (typeof window !== 'undefined') {
      localStorage.setItem('atrios_push_sound_enabled', nextVal ? 'true' : 'false');
      window.dispatchEvent(new CustomEvent('atrios_push_sound_changed', { detail: { enabled: nextVal } }));
    }
  };
  ...
  useEffect(() => {
    const checkExpiry = () => {
      ...
      if (u && u.subscriptionExpiresAt && u.plan !== PlanType.FREE) {
        ...
        if (expiryDate.getTime() < today.getTime()) {
          ...
          alert("O seu período de teste/subscrição expirou. A sua conta foi revertida para o plano Grátis.");
          
          triggerPushNotificationSubmit(
            "Período Expirado ❌",
            "O seu tempo expirou e a sua conta foi revertida para o plano Grátis."
          );
          return;
        }
        ...
        const isDismissed = sessionStorage.getItem('atrios_expiry_banner_dismissed') === 'true';

        if (!isDismissed) {
          if (u.plan === PlanType.PREMIUM_MONTHLY && diffDays <= 5 && diffDays > 0) {
          ...
          } else if (u.plan === PlanType.PREMIUM_ANNUAL && diffDays <= 30 && diffDays > 0) {
            setShowExpiryAlert(true);
            if (!sessionStorage.getItem('notified_expiry_push')) {
              triggerPushNotificationSubmit(
                "Aviso de Assinatura ⏳",
  ...
  const handleDismissExpiryAlert = () => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('atrios_expiry_banner_dismissed', 'true');
    }
  };
  ...
  useEffect(() => {
    if (currentUser && view === 'app') {
      ...
      const companyChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            // Notificações sobre alteração de acesso
            if (currentUserRef.current) {
              const prevCanEdit = Boolean(currentUserRef.current.canEditSensitiveData);
              const newCanEdit = Boolean(updated.canEditSensitiveData);
              if (!prevCanEdit && newCanEdit) {
              ...
              } else if (prevCanEdit && !newCanEdit) {
                triggerPushNotificationSubmit(
                  "Acesso Bloqueado 🔐",
                  "O seu tempo de acesso aos dados sensíveis expirou ou foi bloqueado."
                );
              }
            }
            ...
            if (payload && payload.payload) {
              ...
              if (currentUserRef.current) {
                ...
                if (newCanEdit) {
                  ...
                  triggerPushNotificationSubmit(
                    "Acesso Bloqueado 🔐",
                    "O seu tempo de acesso aos dados sensíveis expirou ou foi bloqueado pelo suporte."
                  );
                }
              }
            }
      ...
      // Sincronização de Solicitações de Orçamentos de 10 em 10 minutos (para economizar gás do banco de dados)
      const fetchBudgetsThrottled = async () => {
        const currentId = currentUserRef.current?.id;
        if (!currentId) return;
        try {
          console.log('[Budget Sync] Sincronizando solicitações de orçamentos (intervalo de 10 min)...');
          const { data: cloudBudgets } = await safeFetch<any[]>(
            supabase.from('budgets').select('*').eq('company_id', currentId)
          );
          if (cloudBudgets && Array.isArray(cloudBudgets)) {
            const mapped = cloudBudgets.map(mapBudgetFromSupabase).filter(Boolean);
            const allBudgets = getAllStoredBudgets();
            const otherBudgets = allBudgets.filter(b => String(b.companyId) !== String(currentId));
            safeSetItem('atrios_budgets', JSON.stringify([...otherBudgets, ...mapped]));
            setBudgets(mapped);
          }
        } catch (err) {
          console.warn('[Budget Sync] Erro na sincronização de 10 minutos:', err);
        }
      };

      // Executa 1ª sincronização e agenda para cada 10 minutos (600.000 ms)
      fetchBudgetsThrottled();
      const budgetSyncInterval = setInterval(fetchBudgetsThrottled, 10 * 60 * 1000);
      ...
      const fallback = setInterval(async () => {
        ...
        // 1. Polling de status e permissão da empresa em tempo real na cloud (a cada 2s)
        try {
          const { data: cloudComp } = await safeFetch<any>(
            supabase.from('companies').select('*').eq('id', currentId).maybeSingle()
          );
          if (cloudComp) {
            const mappedComp = mapCompanyFromSupabase(cloudComp);
            const curr = currentUserRef.current;
            if (curr && mappedComp) {
              const prevCanEdit = Boolean(curr.canEditSensitiveData);
              const newCanEdit = Boolean(mappedComp.canEditSensitiveData);

              if (prevCanEdit !== newCanEdit) {
                console.log(`[Realtime Lock/Unlock] Status alterado de ${prevCanEdit} para ${newCanEdit}`);
                const updatedUser: Company = {
                  ...curr,
                  ...mappedComp,
                  canEditSensitiveData: newCanEdit,
                  unlockRequested: false
                ...
                curr.plan !== mappedComp.plan ||
                curr.unlockRequested !== mappedComp.unlockRequested ||
                curr.subscriptionExpiresAt !== mappedComp.subscriptionExpiresAt
              ) {
                const updatedUser: Company = {
                  ...curr,
                  ...mappedComp
          ...
          console.warn('Erro ao verificar status em tempo real da empresa:', compErr);
        }
  ...
  const handleRequestUnlock = async () => {
    ...
    triggerPushNotificationSubmit(
      "Solicitação de Desbloqueio Enviada! 🔑",
      "A sua solicitação para editar dados nas Definições foi enviada com sucesso à nossa equipa."
    );
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if ((cleanEmail === 'atriossoftware@gmail.com' || cleanEmail === 'jeferson.goes36@gmail.com') && (cleanPass === 'izalivjeh' || password === 'izalivjeh')) {
      saveSession(null, 'master', activeTab, currencyCode);
      setView('master');
      return;
    }
    ...
    const firstLoginAt = companyMapped.firstLoginAt || (companyData as any).first_login_at || (companyData as any).firstLoginAt || companyMapped.lastSeenAt || nowIso;
    ...
    (company as any).first_login_at = firstLoginAt;

    if (!companyData.firstLoginAt && !(companyData as any).first_login_at) {
      safeFetch(supabase.from('companies').update({ firstLoginAt }).eq('id', company.id)).catch(() => {});
    }
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      triggerPushNotificationSubmit(
        ...
        `Pedido de recuperação de acesso para ${email} enviado com sucesso à nossa equipa.`
      );
      ...
      }).catch(err => console.warn('Erro ao notificar master de reset de senha:', err));
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    triggerPushNotificationSubmit(
      ...
      `Bem-vindo(a) ao Átrios App, ${cleanCompanyName}! A sua conta está ativa e pronta a usar.`
    );
  ...
  if (view === 'master') {
    return (
      <>
        ...
        <MasterPanel onLogout={() => { saveSession(null); setView('landing'); }} locale={locale} />
      </>
    );
  }
  ...
  return (
    <>
      ...
      {showExpiryAlert && !expiryAlertDismissed && daysRemaining !== null && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[10000] bg-amber-600 text-white px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 animate-in slide-in-from-top duration-700 border border-amber-400">
           ...
           <button onClick={handleDismissExpiryAlert} className="ml-4 p-2 hover:bg-white/10 rounded-full" title="Fechar aviso"><X size={20} /></button>
        </div>
      )}
      ...
      {view === 'client-live-chat' ? (
        ...
        <LandingPage
          ...
          isIntroActive={showFullscreenIntro}
        />
      ) : view === 'login' ? (
        <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
          ...
          <div className="w-full max-w-md space-y-6 p-6 sm:p-8 bg-white rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-6 duration-500 relative z-10">
            <div className="text-center space-y-2">
              <div className="inline-flex w-12 h-12 items-center justify-center bg-amber-500 rounded-2xl text-white shadow-lg shadow-amber-500/20 mb-1">
                <Construction size={22} />
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{t.welcomeBack}</h2>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">{t.enterData}</p>
            </div>
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.emailLabel}</label>
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-slate-900 transition-all font-bold text-sm text-slate-800" placeholder="exemplo@empresa.pt" />
              </div>
              <div className="space-y-1.5">
                ...
                <input type="password" required value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-slate-900 transition-all font-bold text-sm text-slate-800" placeholder="••••••••" />
              </div>
              <button type="submit" className="w-full py-3 sm:py-3.5 bg-slate-900 text-white rounded-xl font-black text-xs sm:text-sm uppercase tracking-wider hover:bg-slate-800 transition-all shadow-lg active:scale-95">{t.loginBtn}</button>
            </form>
            <div className="text-center space-y-4 pt-1">
              <p className="text-slate-500 font-medium text-xs">{t.noAccount} <button onClick={() => setView('signup')} className="font-bold text-amber-600 hover:text-amber-700 underline decoration-2 underline-offset-4">{t.registerHere}</button></p>
              <div className="flex flex-col gap-3">
                <button onClick={() => setView('landing')} className="text-slate-400 font-black uppercase tracking-widest text-[10px] hover:text-slate-900 transition-colors">{t.backToLogin}</button>
        ...
        <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,255,255,1)_0%,rgba(241,245,249,1)_100%)]" />
          <div className="w-full max-w-lg space-y-6 p-6 sm:p-8 bg-white rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-6 duration-500 relative z-10">
            <div className="text-center space-y-2">
              <div className="inline-flex w-12 h-12 items-center justify-center bg-amber-500 rounded-2xl text-white shadow-lg shadow-amber-500/20 mb-1">
                <Construction size={22} />
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{t.createAccount}</h2>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">{t.enterData}</p>
            </div>
            <form onSubmit={handleSignup} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 space-y-1.5">
                ...
                <input type="text" required value={companyName} onChange={e => setCompanyName(e.target.value)} className="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-slate-900 transition-all font-bold text-sm text-slate-800" placeholder="Nome da sua Empresa" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.emailLabel}</label>
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-slate-900 transition-all font-bold text-sm text-slate-800" placeholder="exemplo@empresa.pt" />
              ...
              <div className="sm:col-span-2 pt-1">
                <button type="submit" className="w-full py-3 sm:py-3.5 bg-slate-900 text-white rounded-xl font-black text-xs sm:text-sm uppercase tracking-wider hover:bg-slate-800 transition-all shadow-lg active:scale-95">{t.registerBtn}</button>
              </div>
            </form>
            <div className="text-center space-y-4 pt-1">
              <p className="text-slate-500 font-medium text-xs">{t.haveAccount} <button onClick={() => setView('login')} className="font-bold text-amber-600 hover:text-amber-700 underline decoration-2 underline-offset-4">{t.loginBtn}</button></p>
              <button onClick={() => setView('login')} className="text-slate-400 font-black uppercase tracking-widest text-[10px] hover:text-slate-900 transition-colors">{t.backToLogin}</button>
              ...
              <div className="flex flex-wrap items-center justify-center gap-4 text-[9px] font-black uppercase tracking-widest text-slate-400 pt-2 border-t border-slate-100">
        ...
        <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
          ...
          <div className="w-full max-w-md space-y-6 p-6 sm:p-8 bg-white rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-6 duration-500 relative z-10">
            <div className="text-center space-y-2">
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{t.resetPasswordTitle}</h2>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">{t.resetPasswordDesc}</p>
            </div>
            <form onSubmit={handleResetPassword} className="space-y-4">
              ...
              <button type="submit" className="w-full py-3 sm:py-3.5 bg-slate-900 text-white rounded-xl font-black text-xs sm:text-sm uppercase tracking-wider hover:bg-slate-800 transition-all shadow-lg active:scale-95">{t.sendResetLink}</button>
            </form>
            <div className="text-center space-y-4 pt-1">
              <button onClick={() => setView('login')} className="text-slate-400 font-black uppercase tracking-widest text-[10px] hover:text-slate-900 transition-colors">{t.backToLogin}</button>
              ...
              <div className="pt-4 border-t border-slate-100">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.orContactSupport}</p>
                <a href="mailto:atriossoftware@gmail.com" className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-100 transition-all">
        ...
        <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,255,255,1)_0%,rgba(241,245,249,1)_100%)]" />
          <div className="w-full max-w-md space-y-6 p-6 sm:p-8 bg-white rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-6 duration-500 text-center relative z-10">
            <div className="space-y-2">
              <div className="inline-flex w-12 h-12 items-center justify-center bg-blue-500 rounded-2xl text-white shadow-lg shadow-blue-500/20 mb-1"><Mail size={22} /></div>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{t.verifyEmailTitle}</h2>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">{t.verifyEmailDesc} {email}</p>
            </div>
            <div className="space-y-3 pt-2">
              <button onClick={handleVerify} className="w-full py-3 sm:py-3.5 bg-slate-900 text-white rounded-xl font-black text-xs sm:text-sm uppercase tracking-wider hover:bg-slate-800 transition-all shadow-lg active:scale-95">{t.simulateVerify}</button>
              <button onClick={() => setView('login')} className="text-slate-400 font-black uppercase tracking-widest text-[10px] hover:text-slate-900 transition-colors">{t.backToLogin}</button>
            </div>
          </div>
        </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          ...
                          {(isReportsLocked || isJobsCreationRestricted) && <Lock size={9} className="absolute -top-1 -right-1 text-amber-500" />}
                ...
                <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 lg:space-y-12">
                  ...
                  {activeTab === 'jobs' && currentUser && (
                    isAtriosBuildUser ? (
                      ...
                      <div className="bg-white rounded-[2.5rem] p-8 md:p-12 border border-slate-100 shadow-xl text-center space-y-6 max-w-2xl mx-auto my-8 animate-in fade-in">
                        <div className="w-20 h-20 bg-amber-500/10 text-amber-600 rounded-3xl flex items-center justify-center mx-auto shadow-inner">
                          <Lock size={36} />
                        </div>
                        <div className="space-y-2">
                          <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-amber-500/10 text-amber-700 text-xs font-black uppercase tracking-wider border border-amber-500/20">
                            <Sparkles size={13} /> Funcionalidade em Breve
                          </span>
                          <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">
                            Vagas de Trabalho
                          </h2>
                          <p className="text-slate-500 text-sm font-medium leading-relaxed max-w-md mx-auto">
                            A página de <strong>Vagas de Trabalho</strong> encontra-se temporariamente bloqueada e em preparação. Muito em breve todas as empresas poderão publicar e encontrar profissionais qualificados! 🚀
                          </p>
                        </div>
                        <button
                          onClick={() => setActiveTab('dashboard')}
                          className="px-8 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-wider hover:bg-slate-800 transition-all shadow-md active:scale-95 cursor-pointer"
                        >
                          Voltar ao Painel
                        </button>
                      </div>
                  ...
                  {activeTab === 'work_hours' && currentUser && (
                    <WorkTimeTracker 
                      company={currentUser} 
                      locale={locale} 
                      currencyCode={currencyCode} 
                         ...
                         <div className="space-y-6 lg:space-y-8">
                            ...
                            {currentUser && (
                              <div className="pt-2 space-y-4">
                                ...
                                <div className="p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl lg:rounded-3xl">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      ...
                                      title={showCompanyId ? t.hideId : t.showId}
                                    >
                                      {showCompanyId ? <EyeOff size={16} /> : <Eye size={16} />}
                                      <span className="hidden sm:inline">
                                        {showCompanyId ? t.hideId : t.showId}
                                      </span>
                                    </button>
                                ...
                                {/* Botão de Ativar / Desativar Som do Push */}
                                <div className="p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl lg:rounded-3xl">
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="flex items-center gap-3 min-w-0">
                                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                                        ...
                                        {pushSoundEnabled ? <Volume2 size={20} /> : <VolumeX size={20} />}
                                      </div>
                                      <div className="min-w-0">
                                        <label className="text-xs font-black text-slate-800 uppercase tracking-wider block">
                                          {t.pushSoundLabel || 'Som de Notificações'}
                                        </label>
                                        <p className="text-[11px] font-bold text-slate-500 leading-tight mt-0.5">
                                          {pushSoundEnabled 
                                            ? (t.pushSoundActiveDesc || 'Som ativado para novos alertas') 
                                            : (t.pushSoundMutedDesc || 'Notificações em modo silencioso')}
                                        </p>
                                      </div>
                                    </div>
                                    ...
                                    <button
                                      ...
                                      className={`px-4 py-2.5 rounded-xl border-2 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shrink-0 ${
                                        pushSoundEnabled
                                          ? 'bg-emerald-600 border-emerald-600 text-white shadow-sm hover:bg-emerald-700 active:scale-95'
                                          : 'bg-slate-200 border-slate-300 text-slate-800 hover:bg-slate-300 active:scale-95'
                                      }`}
                                      title={pushSoundEnabled ? (t.pushSoundDisabled || 'Desativar Som') : (t.pushSoundEnabled || 'Ativar Som')}
                                    >
                                      {pushSoundEnabled ? (
                                        <>
                                          <Check size={16} className="text-emerald-200" />
                                          <span className="whitespace-nowrap">{t.pushSoundEnabled || 'Som Ativado'}</span>
                                        </>
                                      ) : (
                                        <>
                                          <VolumeX size={16} className="text-slate-600" />
                                          <span className="whitespace-nowrap">{t.pushSoundDisabled || 'Som Desativado'}</span>
                                        </>
                                      )}
                                    </button>
                                  </div>
                ...
                <div className="space-y-2 col-span-3">
                  ...
                  <div className="pt-2 flex items-center gap-4">
                    {notificationPermission !== 'denied' ? (
                      ...
                      >
                        {locale.startsWith('pt') ? 'Como Ativar?' : (locale.startsWith('es') ? '¿Cómo activar?' : (locale.startsWith('fr') ? 'Comment activer ?' : (locale.startsWith('it') ? 'Come attivare?' : 'How to enable?')))}
                      </button>
                    )}
                    <button 
                      onClick={dismissPushPrompt} 
              ...
              <div className="w-full max-w-md bg-white rounded-3xl p-6 md:p-8 shadow-2xl border border-slate-100 relative overflow-hidden text-slate-900 animate-in zoom-in-95 duration-300">
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl text-left space-y-2 text-xs text-slate-600 leading-relaxed font-medium w-full">
                    <p>
                      A funcionalidade de <strong>Vagas de Trabalho</strong> encontra-se temporariamente bloqueada e em fase de preparação.
                    </p>
