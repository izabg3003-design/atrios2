...
import { 
  ...
  getStoredBudgets, 
  saveCompany,
  saveCustomServiceToCloud,
  removeCustomServiceFromCloud
...
const BudgetForm: React.FC<BudgetFormProps> = ({ company, onSave, onCancel, onUpgrade, initialData, locale, currencyCode }) => {
  ...
  const [customCategories, setCustomCategories] = useState<{ id: string; name: string }[]>(() => {
    try {
      ...
      // 1. Prioridade para serviços salvos no objeto da empresa logada (sincronizado com Supabase)
      ...
      // 2. Cache local do localStorage específico desta empresa
      const companyKey = company?.id ? `atrios_custom_service_categories_${company.id}` : null;
      const stored = companyKey ? localStorage.getItem(companyKey) : localStorage.getItem('atrios_custom_service_categories');
      if (stored) {
        try {
      ...
      // 4. Também examina orçamentos locais exclusivamente da empresa atual
      if (company?.id) {
    ...
    if (company?.customServices && Array.isArray(company.customServices)) {
      ...
      setCustomCategories(prev => {
        ...
        if (changed) {
          try {
            if (company?.id) {
              localStorage.setItem(`atrios_custom_service_categories_${company.id}`, JSON.stringify(updated));
            }
  ...
  }, [company?.customServices, company?.id]);
  ...
  const handleAddCustomCategory = (e?: React.FormEvent) => {
    ...
    // Sincroniza diretamente na tabela service_categories e no perfil da empresa
    if (company?.id) {
      saveCustomServiceToCloud(company.id, { id, name: cleanName }).catch(err => {
        console.warn("Erro ao salvar serviço em service_categories:", err);
      });
  ...
  const handleRemoveCustomCategory = (categoryId: string, e: React.MouseEvent) => {
    ...
    if (company?.id) {
      removeCustomServiceFromCloud(company.id, categoryId).catch(err => {
        console.warn("Erro ao remover serviço de service_categories:", err);
      });
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-4">
                ...
                {!isLocked && (
                  ...
                  >
                    ...
                    <span className="text-[9px] font-black uppercase tracking-wider text-slate-500 group-hover:text-amber-500 transition-colors">{t.newCategory}</span>
                  </button>
                )}
              </div>
            </section>
