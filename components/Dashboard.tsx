  ...
  return (
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-8">
        <div className="bg-white p-6 lg:p-8 rounded-[1.5rem] lg:rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-4 lg:gap-6 group hover:shadow-xl hover:shadow-emerald-500/10 transition-all">
          <div className="p-4 lg:p-5 bg-emerald-50 text-emerald-600 rounded-xl lg:rounded-[1.5rem] group-hover:scale-110 transition-transform">
            <CheckCircle size={24} className="lg:w-8 lg:h-8" />
          </div>
          <div>
            <p className="text-[8px] lg:text-[10px] text-slate-400 font-black uppercase tracking-widest lg:tracking-[0.2em] mb-0.5 lg:mb-1">{t.totalApproved}</p>
            <p className="text-lg lg:text-2xl font-black text-slate-900">{stats.totalApproved.toLocaleString(locale, { style: 'currency', currency: currencyCode })}</p>
          </div>
        </div>
        <div className="bg-white p-6 lg:p-8 rounded-[1.5rem] lg:rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-4 lg:gap-6 group hover:shadow-xl hover:shadow-amber-500/10 transition-all">
          <div className="p-4 lg:p-5 bg-amber-50 text-amber-600 rounded-xl lg:rounded-[1.5rem] group-hover:scale-110 transition-transform">
            <Clock size={24} className="lg:w-8 lg:h-8" />
          </div>
          <div>
            <p className="text-[8px] lg:text-[10px] text-slate-400 font-black uppercase tracking-widest lg:tracking-[0.2em] mb-0.5 lg:mb-1">{t.totalPending}</p>
            <p className="text-lg lg:text-2xl font-black text-slate-900">{stats.totalPending.toLocaleString(locale, { style: 'currency', currency: currencyCode })}</p>
          </div>
        </div>
        <div className="bg-white p-6 lg:p-8 rounded-[1.5rem] lg:rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-4 lg:gap-6 group hover:shadow-xl hover:shadow-blue-500/10 transition-all">
          <div className="p-4 lg:p-5 bg-blue-50 text-blue-600 rounded-xl lg:rounded-[1.5rem] group-hover:scale-110 transition-transform">
            <DollarSign size={24} className="lg:w-8 lg:h-8" />
          </div>
          <div>
            <p className="text-[8px] lg:text-[10px] text-slate-400 font-black uppercase tracking-widest lg:tracking-[0.2em] mb-0.5 lg:mb-1">{t.totalReceived}</p>
            <p className="text-lg lg:text-2xl font-black text-slate-900">{stats.totalReceived.toLocaleString(locale, { style: 'currency', currency: currencyCode })}</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 ${!isPremium ? 'blur-[4px] pointer-events-none grayscale' : ''}`}>
          <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[2rem] lg:rounded-[3rem] shadow-sm border border-slate-100">
            <h3 className="text-lg lg:text-xl font-black text-slate-900 mb-6 lg:mb-10 flex items-center gap-2 lg:gap-3 uppercase tracking-tight">
              <TrendingUp size={20} className="text-blue-500 lg:w-6 lg:h-6" />
              {t.volumeChartTitle}
            </h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Tooltip 
                    ...
                    contentStyle={{borderRadius: '20px', border: 'none', boxShadow: '0 25px 50px -12px rgb(0 0 0 / 0.1)', padding: '12px', fontWeight: 'bold', fontSize: '12px'}}
                  />
                  <Bar dataKey="value" radius={[8, 8, 0, 0]} barSize={30} lg:barSize={40}>
          ...
          <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[2rem] lg:rounded-[3rem] shadow-sm border border-slate-100">
            <h3 className="text-lg lg:text-xl font-black text-slate-900 mb-6 lg:mb-10 flex items-center gap-2 lg:gap-3 uppercase tracking-tight">
              <CheckCircle size={20} className="text-emerald-500 lg:w-6 lg:h-6" />
              {t.statusChartTitle}
            </h3>
            <div className="h-[250px] lg:h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="flex flex-wrap justify-center gap-4 lg:gap-8 mt-4">
                {pieData.map(d => (
                  <div key={d.name} className="flex items-center gap-2 lg:gap-3">
                    ...
                    <span className="text-[8px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">{d.name}</span>
                  </div>
                ))}
              </div>
            </div>
