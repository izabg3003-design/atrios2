...
export const JobOffers: React.FC<JobOffersProps> = ({ company, locale = 'pt-PT' }) => {
  ...
  const handleOpenModal = (offerToEdit?: JobOffer) => {
    if (!canCreateJobOffers) {
      alert('A funcionalidade de inclusão e publicação direta de vagas de trabalho estará disponível em breve no Átrios Work. Fique atento às novidades!');
      return;
    }
    if (offerToEdit) {
    ...
    <div className="space-y-6 sm:space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          {canCreateJobOffers ? (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black shadow-lg shadow-amber-500/20 transition-all active:scale-95 text-sm shrink-0 cursor-pointer"
            >
              <Plus size={18} /> {t.publishJob}
            </button>
          ) : (
            <div className="flex items-center gap-3.5 px-5 py-4 rounded-2xl bg-slate-900/90 border border-amber-500/30 text-slate-200 font-medium shrink-0 max-w-md shadow-xl backdrop-blur-md">
              <div className="p-2.5 rounded-xl bg-amber-500/20 text-amber-400 shrink-0 border border-amber-500/30">
                <Sparkles size={20} />
              </div>
              <div>
                <p className="text-amber-300 font-extrabold text-xs uppercase tracking-wider mb-0.5 flex items-center gap-1.5">
                  Em Breve no Átrios Work
                </p>
                <p className="text-slate-300 text-xs leading-relaxed font-medium">
                  A publicação e gestão direta de vagas de trabalho estará disponível em breve para a sua conta. Fique atento às próximas novidades!
                </p>
              </div>
            </div>
          )}
      ...
      {filteredOffers.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-100 shadow-sm space-y-4">
          ...
          {statusFilter === 'all' && searchTerm === '' && canCreateJobOffers && (
            <button
              onClick={() => handleOpenModal()}
              className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-slate-900 text-white text-xs font-black hover:bg-slate-800 transition-all cursor-pointer"
            >
