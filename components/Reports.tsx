  ...
  const chartData = useMemo(() => {
    ...
    if (selectedPeriod === 'weekly') {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dayBudgets = periodSales.filter(s => (s.createdAt || s.created_at || '').startsWith(dayStr));
      ...
      for (let i = 0; i < 12; i++) {
        const monthBudgets = periodSales.filter(s => new Date(s.createdAt || s.created_at).getMonth() === i);
  ...
  return (
    <div className="space-y-8 lg:space-y-12 animate-in fade-in duration-700 pb-20">
          ...
          <div className="h-[200px] sm:h-[300px] lg:h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 8, fontWeight: 'bold'}} />
                <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 8, fontWeight: 'bold'}} />
                <Tooltip 
                  cursor={{fill: '#f8fafc'}}
                  contentStyle={{borderRadius: '15px', border: 'none', boxShadow: '0 25px 50px -12px rgb(0 0 0 / 0.1)', padding: '8px', fontSize: '10px'}}
        ...
        <div className="bg-white p-5 sm:p-8 lg:p-10 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[3rem] shadow-sm border border-slate-100 flex flex-col items-center justify-between text-center gap-6">
           ...
           <div className="h-[180px] sm:h-[200px] lg:h-[250px] w-full relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center flex-col">
                ...
                <span className="text-lg sm:text-xl lg:text-2xl font-black text-emerald-600">{stats.margin.toFixed(0)}%</span>
              </div>
           </div>
           <div className="w-full space-y-2 sm:space-y-3 lg:space-y-4 pt-4 lg:pt-6 border-t border-slate-50">
              <div className="flex justify-between items-center">
                <span className="text-[7px] sm:text-[8px] lg:text-[10px] font-black text-slate-400 uppercase">{t.reportRevenueTotal}</span>
                <span className="text-[10px] sm:text-xs lg:text-sm font-black text-slate-900">{stats.revenue.toLocaleString(locale, { style: 'currency', currency: currencyCode })}</span>
              </div>
              <div className="flex justify-between items-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 lg:gap-6">
        ...
        <div className="bg-slate-900 p-5 sm:p-6 lg:p-8 rounded-[1.25rem] sm:rounded-[1.5rem] lg:rounded-[2.5rem] shadow-2xl space-y-3 lg:space-y-4 group relative overflow-hidden">
              ...
              periodSales.map(budget => (
                <div key={budget.id} className="bg-white p-4 lg:p-6 rounded-[1.5rem] lg:rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 lg:gap-6 group hover:border-slate-300 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[8px] lg:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{new Date(budget.createdAt || budget.created_at).toLocaleDateString(locale)}</p>
                  </div>
