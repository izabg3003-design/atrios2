...
import { triggerInAppBalloon } from './InAppPushBalloon';
...
export const Store: React.FC<StoreProps> = ({ t, locale, companyId, companyName, companyEmail, orders }) => {
  ...
  const confirmQuoteRequest = async () => {
    ...
    try {
      ...
      triggerInAppBalloon(
        "Pedido de Orçamento Enviado! 🛍️",
        `O seu pedido (${quantity}x ${selectedProduct?.name || 'Produto'}) foi submetido com sucesso à nossa equipa.`
      );
