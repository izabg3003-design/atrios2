      ...
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-8 py-6 bg-slate-900 text-white flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">{t.expensesTitle}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
          <div className="p-6 bg-red-50 rounded-2xl border border-red-100 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold text-red-600 uppercase tracking-widest mb-1">{t.totalExpenses}</p>
              <p className="text-3xl font-black text-red-700">{formatValue(totalExpenses)}</p>
            </div>
          ...
          <div className="space-y-4">
            ...
            {showForm && (
              <form onSubmit={handleAddExpense} className="p-6 bg-slate-50 rounded-2xl border border-slate-200 animate-in slide-in-from-top duration-300">
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">{t.expenseDescription}</label>
                    <input required type="text" value={description} onChange={e => setDescription(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-slate-900 font-bold" placeholder={t.expensePlaceholder} />
                  </div>
                  
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1"><Hash size={10}/> {t.quantity}</label>
                      <input 
                        required 
                        type="number" 
                        value={quantity === 0 ? '' : quantity} 
                        onChange={e => setQuantity(e.target.value === '' ? 0 : Number(e.target.value))} 
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none font-bold" 
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1"><Layers size={10}/> {t.unit}</label>
                      <input 
                        required 
                        type="text" 
                        value={unit} 
                    ...
                    <div className="col-span-1 sm:col-span-2">
                      <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1"><DollarSign size={10}/> {t.unitPrice} ({currencyCode})</label>
                      <input 
                        required 
                        type="number" 
                        step="0.01" 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">{t.expenseDate}</label>
                      <input required type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none font-bold" />
                    </div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button type="button" onClick={() => setShowForm(false)} className="px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold">
                    {t.cancel}
                  </button>
                </div>
              </form>
            ...
            <div className="space-y-3">
              {(budget.expenses || []).map(expense => (
                <div key={expense.id} className="p-4 bg-white border border-slate-100 rounded-2xl flex items-center justify-between group shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-red-50 text-red-600 rounded-full flex items-center justify-center font-bold">{currencyInfo.symbol}</div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    <p className="font-black text-red-600">-{formatValue(expense.amount)}</p>
                    <button onClick={() => removeExpense(expense.id)} className="p-2 text-slate-200 hover:text-red-500 transition-all"><Trash2 size={18} /></button>
                  </div>
                </div>
              ))}
            </div>
