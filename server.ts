...
// Helper para limpar automaticamente subscrições inativas muito antigas (com mais de 180 dias) do Supabase
async function pruneOldSubscriptionsFromSupabase() {
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) return;
  try {
    const hundredEightyDaysAgo = new Date(Date.now() - 180 * 24 * 60 * 60 * 1000).toISOString();
    supabase
      .from("push_subscriptions")
      .delete()
      .lt("created_at", hundredEightyDaysAgo)
      .then(({ error }) => {
        if (!error) console.log("[Supabase Prune] Subscrições inativas (>180 dias) limpas.");
...
const SUB_CACHE_TTL = 30000; // 30 segundos de cache em memória
...
// Helper resiliente para buscar subscrições do Supabase com cache e timeout seguro (5000ms)
async function fetchSubscriptionsFromSupabase(): Promise<{ web: any[], fcm: any[] }> {
  ...
  try {
    ...
    const timeoutPromise = new Promise((resolve) => setTimeout(() => resolve({ data: null, timeout: true }), 5000));
    
    const result: any = await Promise.race([fetchPromise, timeoutPromise]);
    if (result.timeout || result.error) {
      console.warn("[Supabase Fetch Subs] Timeout/error lendo subscrições. Usando cache ou fallback.");
      return cachedPushSubs || empty;
    }
    ...
    if (result.data && Array.isArray(result.data)) {
      ...
      console.log(`[Supabase Fetch Subs] Carregadas ${cachedPushSubs.web.length} subscrições Web Push e ${cachedPushSubs.fcm.length} FCM do banco.`);
    }
  } catch (err: any) {
